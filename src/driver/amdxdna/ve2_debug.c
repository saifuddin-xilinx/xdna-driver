...
static struct device *get_aie_device_handle(struct amdxdna_dev *xdna, u32 col, u32 *lead_col)
{
	struct amdxdna_ctx *hwctx;
				   ...
				   struct amdxdna_drm_set_state *args)
{
	...
	aie_dev = get_aie_device_handle(xdev, info.col, &lead_col);
	if (!aie_dev) {
		XDNA_ERR(xdev, "AIE device handle not found for given col %u\n", info.col);
		return -EINVAL;
	}

	ret = ve2_partition_write(aie_dev, lead_col, info.col, info.row, info.addr,
...
static int ve2_tile_data_reg_read(struct amdxdna_client *client, struct amdxdna_drm_get_info *args)
{
	...
	ret = ve2_partition_read(aie_dev, lead_col, info.col, info.row, info.addr,
					  sizeof(uint32_t),
					  (void *)&(info.val));
